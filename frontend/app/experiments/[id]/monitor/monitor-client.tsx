      ...
      <Tabs defaultValue="monitor">
        ...
        <TabsContent value="monitor" className="space-y-4 mt-4">
          ...
          {experiment.status === "running" && experiment.progress && (() => {
            ...
            const pct = p.rows_total > 0 ? Math.round((p.rows_done / p.rows_total) * 100) : 0;
            const etaMin = Math.ceil(p.eta_s / 60);
            const phaseLabel = p.phase === "smbo" ? "SMBO polish" : `GP · gen ${p.generation}`;
            return (
              <Card>
                <CardHeader className="pb-2 flex flex-row items-center justify-between">
                  <CardTitle className="text-sm text-muted-foreground">Current Trial Progress</CardTitle>
                  <Badge variant="outline">{phaseLabel}</Badge>
                </CardHeader>
                <CardContent className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span>Row {p.rows_done} / {p.rows_total}</span>
                    <span>{etaMin > 0 ? `~${etaMin} min remaining` : "finishing…"}</span>
                  </div>
